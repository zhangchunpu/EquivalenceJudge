...
# elem == root xml.etree.ElementTree により得たオブジェクト
        ...
        #semanticsで囲まれた要素は一個以上ある場合もあるが、一個目だけをcls.parse_from_mathmlに突っ込むのはなぜ？
            ...
            #list(map(cls.parse_from_mathml, elem))のnot MathSymbol.ignore_tag(x)を満たす要素を取り出す、返り値はリストではなく、filterオブジェクトである.
